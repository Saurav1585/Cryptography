...
def des(inputt,key):
	...
	left=''.join(left)
	right=''.join(right)
	print("Initial\t",hex(int(left, 2)).upper(),"\t",hex(int(right, 2)).upper(),"\t (Initial Permutation)\n")
	for i in range(16):
		...
		print(i+1,"\t",hex(int(left, 2)).upper(),"\t",hex(int(right, 2)).upper(),"\t",hex(int(key[15-i], 2)).upper())
	text=left+right
	output=final(text)
